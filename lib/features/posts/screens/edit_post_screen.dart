...
class _EditPostScreenState extends ConsumerState<EditPostScreen> {
  ...
  void editPost(Post post) {
    if (selectedImages.length <= 10 &&
      ...
      ref.read(postContollerProvider.notifier).editPost(
            oldPost: post,
            context: context,
      ...
      showSnackBar(context, "Будь ласка, перевірте введені дані");
    }
  }
  ...
  Widget build(BuildContext context) {
    final loading = ref.watch(postContollerProvider);
    return loading
        ? const Loader()
        : ref.watch(getPostByIdProvider(widget.postId)).when(
            data: (data) {
              print(data);
              if (inited) {
                _nameController.text = data.title;
                _addressController.text = data.address ?? "";
                _contactController.text = data.contacts;
                _descriptionController.text = data.description;
                _isChecked = data.isFree;
                _priceController.text =
                    data.price == null ? "" : data.price.toString();
                selectedImages.addAll(data.pictures);
                inited = false;
              }
              print(data);
              return Scaffold(
                ...
                appBar: AppBar(
                  title: const Text('Змінити оголошення'),
                  backgroundColor: const Color(0xFF16382B),
                  leading: IconButton(
                    onPressed: () {
                      goBackConfirmation();
                            ...
                            child: Column(
                              ...
                              children: [
                                TextFormField(
                                  ...
                                  style:
                                      const TextStyle(color: Color(0xFFFEF6EA)),
                                ...
                                builder: (context) {
                                  return AlertDialog(
                                    ...
                                    title: const Text(
                                      'Помилка',
                                      style:
                                          TextStyle(color: Color(0xFFFFCE0C)),
                                    ),
                                    content: const Text(
